...
//1. splay next
//2. splay x
//3. detach, if x is the biggest - detach another way
void erase(
  // Vertex* &root, 
  int k) {                   
  Vertex *x = find(root, k);
  if (x == NULL || x->key != k)
    return;
  //find next - bigger than x
  Vertex* next = NULL;
  if (x->right != NULL)
    next = x->right;
  else {
  ...
  //if next == NULL then x is the biggest
  if (next == NULL) {
    //it is on the tail
    if (x->parent != NULL) {
      x->parent->right = NULL;
...
long long in_order(Vertex* x) { 
  long long sum = 0;
  if (x->left != NULL)
    sum += in_order(x->left);
  sum += x->key;
  if (x->right != NULL)
    sum += in_order(x->right);
    return sum;
}
